...
var iTimeUniform, 
    ...
    vel_iTimeUniform, vel_iResolution, vel_iMouse, vel_keyPoints, 
    trail_opacity;
...
function draw(){
  ...
  for(let i=0; i<keyPoints.length; i++){
    //copy_keyPoints[i] = [keyPoints[i].x*gl.canvas.width/width,keyPoints[i].y*gl.canvas.height/height];
    copy_keyPoints.push(keyPoints[i].x*gl.canvas.width/width);
    copy_keyPoints.push(keyPoints[i].y*gl.canvas.height/height);
  }
  //console.log(copy_keyPoints);
...
function drawToVelTexture(gl,time){
    ...
    if(copy_keyPoints.length>16){
      ...
      //gl.uniform2fv(vel_keyPoints,new Float32Array([width/2,height/2]));
    }
